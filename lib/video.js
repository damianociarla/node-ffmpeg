...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	/**
	 * Sets the new audio format
	 */
	this.setAudioFormat = function (format) {
		// Check if the format is supported by ffmpeg version
		if (this.info_configuration.encode.indexOf(format) != -1) {
			// Check if format is equal 'MP3' and check if the version of ffmpeg support the libmp3lame function
			if (format == 'mp3' && this.info_configuration.modules.indexOf('libmp3lame') != -1)
				format = 'libmp3lame';
			
			if (options.audio == undefined)
				options.audio = new Object();
			// Set the new option
			options.audio.format = format;
			return this;
		} else 
			throw errors.renderError('format_not_supported', format);
	}
	 ...
	 */
	this.setSize = function (size, keepPixelAspectRatio, keepAspectRatio, paddingColor) {
		if (options.video == undefined)
			options.video = new Object();
		// Set the new option
	...
	this.setWatermark = function (watermarkPath, settings) {
		...
		if (baseSettings.margin_top == null || isNaN(baseSettings.margin_top))
			baseSettings.margin_top = 0;
		...
		var getVerticalMargins = function (top, sud) {
			return getSing(top, false).toString() + getSing(sud, true).toString();
		}
		
		// Calculate formula		
		switch (baseSettings.position) {
			case 'NE':
				overlay = '0' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':0' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'NC':
				overlay = 'main_w/2-overlay_w/2' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':0' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'NW':
				overlay = 'main_w-overlay_w' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':0' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'SE':
				overlay = '0' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':main_h-overlay_h' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'SC':
				overlay = 'main_w/2-overlay_w/2' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':main_h-overlay_h' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'SW':
				overlay = 'main_w-overlay_w' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':main_h-overlay_h' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'CE':
				overlay = '0' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':main_h/2-overlay_h/2' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'C':
				overlay = 'main_w/2-overlay_w/2' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':main_h/2-overlay_h/2' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
			case 'CW':
				overlay = 'main_w-overlay_w' + getHorizontalMargins(baseSettings.margin_east, baseSettings.margin_west) + ':main_h/2-overlay_h/2' + getVerticalMargins(baseSettings.margin_top, baseSettings.margin_sud);
				break;
		}
	...
	this.save = function (destionationFileName, callback) {
		...
		if (options.hasOwnProperty('audio')) {
			...
			if (options.audio.hasOwnProperty('disabled')) {
				...
				if (options.audio.hasOwnProperty('format'))
					this.addCommand('-acodec', options.audio.format);
				...
				if (options.audio.hasOwnProperty('bitrate'))
					this.addCommand('-ar', parseInt(options.audio.bitrate, 10) + 'kb');				
			}
		}
	...
	this.fnExtractSoundToMP3 = function (destionationFileName, callback) {
		...
		this.addCommand('-i', this.file_path);
		this.addCommand('-vn');
	...
	this.fnExtractFrameToJPG = function (/* destinationFolder, settings, callback */) {
		...
		this.setSize(settings.size, settings.keep_pixel_aspect_ratio, settings.keep_aspect_ratio, settings.padding_color);
		// Get the dimensions
		var newDimension = _calculateNewDimension.call(this);
