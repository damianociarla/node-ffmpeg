...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	this.setVideoAspectRatio = function (aspect) {
		...
		if (isNaN(aspect)) {
			// Check if aspect is string with format xx:xx
	...
	this.setAudioCodec = function (codec) {
		// Check if the codec is supported by ffmpeg version
		if (this.info_configuration.encode.indexOf(codec) != -1) {
			// Check if codec is equal to 'MP3' and check if the version of ffmpeg support the libmp3lame function
	...
	var _calculateNewDimension = function () {
		...
		// Check if keeping pixel aspect ratio is required
		...
		if (fixedWidth) {
			...
			// Check if the video's aspect ratio is set
	...
	var execCommand = function (callback, folder) {
		...
		// Building the value for return value. Check if the callback is a function. If it's not create a new instance of the deferred class
		...
		utils.exec(finalCommands, settings, function (error, stdout, stderr) {
			...
			if (!error) {
				...
				if (onlyDestinationFile) {
					...
					// Scan all files and prepend the folder path
					for (var i in result)
	...
	this.fnExtractFrameToJPG = function (/* destinationFolder, settings, callback */) {
		...
		var settings = {
			start_time				: null		// Time to start capturing frames
		  , duration_time			: null		// Duration of recording
		  , frame_rate				: null		// Number of frames to capture per second
		  , size					: null		// Dimensions of each frame
		  , number					: null		// Total number of frames to capture
		  , every_n_frames			: null		// Capture frame after every N frames
		  , every_n_seconds			: null		// Capture frame after every N seconds
		  , every_n_percentage		: null		// Capture frame on every N percentage range
		  ...
		  , file_name				: null		// Images file name (will be followed by number number frame so all images have unique name)
		};
		...
		// Check if settings are specified
		if (newSettings !== null)
		...
		// Check if frame_rate is of type number
		if (settings.frame_rate != null && isNaN(settings.frame_rate))
			settings.frame_rate = null;

		// If there is no size setting then the size of the screenshots is equal to video size
		...
		// If file_name is null then file_name is equal to original filename
		if (settings.file_name == null) {
			...
			// Check if exists replacements. Then scan all replacements and build the final filename
			if (replacements) {
