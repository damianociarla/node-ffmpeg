...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	 ...
	 * Extracting sound from a video, and save it as Mp3
	 */
	this.fnTrimSilenceFromMP3 = function (destionationFileName, callback) {
		// Check if file already exists. In this case will remove it
		if (fs.existsSync(destionationFileName))
			fs.unlinkSync(destionationFileName);
		...
		this.addCommand("-af", `"silenceremove=start_periods=1:start_duration=1:start_threshold=-70dB:detection=peak,aformat=dblp,areverse,silenceremove=start_periods=1:start_duration=1:start_threshold=-70dB:detection=peak,aformat=dblp,areverse"`);
